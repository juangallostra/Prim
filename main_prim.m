...
f=reshape(D,1,n*n);    % writes the distance matrix D in a one dimensional vector
f=f(f~=0);             % deletes variables related with edges ii  (size(f)=n*(n-1))
f=[f,zeros(1,n*(n-1))];% add to the previous vector n*(n-1) coeficients associated to the y variables
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
a1 = ones(1,21);
A1cell = repmat({a1}, 1, 22);
A1 = blkdiag(A1cell{:});

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calculate the matrix A2

A2 = repmat({eye(21)},1,22);
for i=0:21
    k=i+1;
    A2{k} = [A2{k}(1:i,:);zeros(1,21);A2{k}(i+1:end,:)];
end
A2 = cell2mat(A2);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calculate the matrices A3, A4, I4, Z
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Final formulation of constraints
...
lb=zeros(2*n*(n-1),1);                      % lower bounds

ub=[ones(n*(n-1)); inf*ones(n*(n-1))];      % upper bounds

[xmin, fmin, status, extra]=intlinprog(f',1:(2*n*(n-1)),A,b,Aeq,beq,lb,ub);
...
C=reshape(xmin(1:(n*(n-1))),n-1,n);     % Only the first n*(n-1) variables have meaning
B=zeros(n);B(~eye(n))=C;
